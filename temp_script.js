
    const routes = {
      '/setup': 'view-setup',
      '/dashboard': 'view-dashboard',
      '/communities/new': 'view-add-community',
    };

    function navigateTo(url) {
      history.pushState(null, '', url);
      routeToView();
    }

    function routeToView() {
      const path = window.location.pathname;
      document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));

      if (path.startsWith('/simulate/')) {
        document.getElementById('view-simulate').classList.add('active');
        const communityId = parseInt(path.split('/')[2]);
        if (communityId) initSimulateView(communityId);
        return;
      }

      const viewId = routes[path];
      if (viewId) {
        document.getElementById(viewId).classList.add('active');
        if (viewId === 'view-dashboard') initDashboard();
        if (viewId === 'view-setup') initSetup();
        if (viewId === 'view-add-community') initAddCommunity();
        return;
      }

      checkProductAndRoute();
    }

    async function checkProductAndRoute() {
      try {
        const resp = await fetch('/api/product');
        if (resp.ok) { navigateTo('/dashboard'); }
        else { navigateTo('/setup'); }
      } catch (e) { navigateTo('/setup'); }
    }

    window.addEventListener('popstate', routeToView);

    function initSetup() {}

document.getElementById('setup-save-btn').addEventListener('click', async () => {
  const name = document.getElementById('setup-name').value.trim();
  if (!name) { alert('Product name is required'); return; }
  
  const btn = document.getElementById('setup-save-btn');
  btn.disabled = true;
  btn.textContent = 'Saving...';
  
  try {
    const resp = await fetch('/api/product', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        name,
        tagline: document.getElementById('setup-tagline').value.trim() || null,
        description: document.getElementById('setup-description').value.trim() || null,
        features: document.getElementById('setup-features').value.trim() || null,
        pricing: document.getElementById('setup-pricing').value.trim() || null,
        target_audience: document.getElementById('setup-target').value.trim() || null,
        llm_model: document.getElementById('setup-llm-model').value.trim() || null,
        llm_base_url: document.getElementById('setup-llm-url').value.trim() || null,
        llm_api_key: document.getElementById('setup-llm-key').value.trim() || null,
      }),
    });
    if (!resp.ok) throw new Error('Failed to save');
    navigateTo('/dashboard');
  } catch (err) {
    alert('Error: ' + err.message);
  } finally {
    btn.disabled = false;
    btn.textContent = 'Save & Continue';
  }
});

    
async function initDashboard() {
  const [productResp, commResp, runsResp] = await Promise.all([
    fetch('/api/product'),
    fetch('/api/communities'),
    fetch('/api/runs'),
  ]);
  if (!productResp.ok) { navigateTo('/setup'); return; }
  
  const product = await productResp.json();
  const communities = await commResp.json();
  const runs = await runsResp.json();
  
  renderDashboard(product, communities, runs);
  setupSettingsPanel(product);
}

function renderDashboard(product, communities, runs) {
  const container = document.getElementById('dashboard-content');
  const initial = product.name ? product.name.charAt(0).toUpperCase() : 'P';
  
  let html = `
    <div class="product-bar">
      <div class="product-info">
        <div class="product-icon">${initial}</div>
        <div>
          <div class="product-name">${escapeHTML(product.name || 'Your Product')}</div>
          <div class="product-tagline">${escapeHTML(product.tagline || '')}</div>
        </div>
      </div>
      <button class="settings-btn" onclick="document.getElementById('settings-panel').classList.add('open')">Settings</button>
    </div>
    <div class="main">
      <div class="section-title">Communities</div>
      <div class="community-grid">
  `;
  
  communities.forEach(c => {
    const cRuns = runs.filter(r => r.community_id === c.id);
    const lastRun = cRuns.length > 0 ? cRuns[0].created_at : null;
    const timeAgo = lastRun ? new Date(lastRun).toLocaleDateString() : 'Never';
    
    html += `
      <div class="community-card">
        <div class="community-name">${escapeHTML(c.subreddit)}</div>
        <div class="community-meta">
          <span>${c.profile_count} personas &middot; ${cRuns.length} runs</span>
          <span>Last run: ${timeAgo}</span>
        </div>
        <button class="simulate-btn" onclick="navigateTo('/simulate/' + ${c.id})">Simulate</button>
      </div>
    `;
  });
  
  html += `
        <div class="community-card add-new" onclick="navigateTo('/communities/new')">
          <div class="add-icon">+</div>
          <div class="add-label">Add community</div>
        </div>
      </div>
      
      <div class="section-title">Recent Runs</div>
  `;
  
  if (runs.length === 0) {
    html += '<div class="empty-state">No runs yet. Simulate a community to get started.</div>';
  } else {
    html += `
      <table class="runs-table">
        <thead>
          <tr>
            <th>Run</th>
            <th>Community</th>
            <th>Grade</th>
            <th>Comments</th>
            <th>Agents</th>
            <th>When</th>
          </tr>
        </thead>
        <tbody>
    `;
    
    runs.forEach(r => {
      const c = communities.find(comm => comm.id === r.community_id);
      const cName = c ? c.subreddit : 'Unknown';
      const score = r.score || 0;
      let gradeStr = 'D', gradeClass = 'red';
      if (score >= 8) { gradeStr = 'A'; gradeClass = 'green'; }
      else if (score >= 6) { gradeStr = 'B'; gradeClass = 'green'; }
      else if (score >= 4) { gradeStr = 'C'; gradeClass = 'yellow'; }
      const date = new Date(r.created_at).toLocaleDateString();
      
      html += `
        <tr onclick="navigateTo('/simulate/' + ${r.community_id} + '?run=' + '${r.tag}')">
          <td><span class="status-dot complete"></span><span class="tag-text">${r.tag}</span></td>
          <td><span class="subreddit-label">${escapeHTML(cName)}</span></td>
          <td><span class="grade-badge ${gradeClass}">${gradeStr}</span></td>
          <td>${r.comments_count || 0}</td>
          <td>${r.agent_count || 0}</td>
          <td class="time-ago">${date}</td>
        </tr>
      `;
    });
    
    html += `
        </tbody>
      </table>
    `;
  }
  
  html += '</div>';
  container.innerHTML = html;
}

function setupSettingsPanel(product) {
  document.getElementById('dash-setup-name').value = product.name || '';
  document.getElementById('dash-setup-tagline').value = product.tagline || '';
  document.getElementById('dash-setup-description').value = product.description || '';
  document.getElementById('dash-setup-features').value = product.features || '';
  document.getElementById('dash-setup-pricing').value = product.pricing || '';
  document.getElementById('dash-setup-target').value = product.target_audience || '';
  document.getElementById('dash-setup-llm-model').value = product.llm_model || 'gpt-5-mini';
  document.getElementById('dash-setup-llm-url').value = product.llm_base_url || '';
  document.getElementById('dash-setup-llm-key').value = product.llm_api_key || '';
}

document.getElementById('dash-save-btn').addEventListener('click', async () => {
  const name = document.getElementById('dash-setup-name').value.trim();
  if (!name) { alert('Product name is required'); return; }
  
  const btn = document.getElementById('dash-save-btn');
  btn.disabled = true;
  btn.textContent = 'Saving...';
  
  try {
    const resp = await fetch('/api/product', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        name,
        tagline: document.getElementById('dash-setup-tagline').value.trim() || null,
        description: document.getElementById('dash-setup-description').value.trim() || null,
        features: document.getElementById('dash-setup-features').value.trim() || null,
        pricing: document.getElementById('dash-setup-pricing').value.trim() || null,
        target_audience: document.getElementById('dash-setup-target').value.trim() || null,
        llm_model: document.getElementById('dash-setup-llm-model').value.trim() || null,
        llm_base_url: document.getElementById('dash-setup-llm-url').value.trim() || null,
        llm_api_key: document.getElementById('dash-setup-llm-key').value.trim() || null,
      }),
    });
    if (!resp.ok) throw new Error('Failed to save');
    document.getElementById('settings-panel').classList.remove('open');
    initDashboard();
  } catch (err) {
    alert('Error: ' + err.message);
  } finally {
    btn.disabled = false;
    btn.textContent = 'Save Settings';
  }
});

    
function initAddCommunity() {
  fetch('/api/product').then(r => r.json()).then(p => {
    document.getElementById('add-comm-product-name').textContent = p.name || 'FlowPulse';
  }).catch(() => {});
  
  document.getElementById('new-subreddit-input').value = 'r/';
  document.getElementById('new-personas-section').classList.add('hidden');
  document.getElementById('new-generating-state').classList.add('hidden');
  document.getElementById('step1-num').classList.add('active');
  document.getElementById('step1-num').classList.remove('done');
  document.getElementById('step1-num').innerHTML = '1';
  document.getElementById('step2-num').classList.remove('active');
  document.getElementById('step2-title').style.color = 'var(--muted)';
  document.getElementById('new-generate-btn').style.display = 'block';
  document.getElementById('new-subreddit-input').disabled = false;
  document.getElementById('new-persona-slider').disabled = false;
}

document.getElementById('new-persona-slider').addEventListener('input', (e) => {
  document.getElementById('new-persona-val').textContent = e.target.value;
});

document.getElementById('new-generate-btn').addEventListener('click', async () => {
  const subreddit = document.getElementById('new-subreddit-input').value.trim();
  if (!subreddit || subreddit === 'r/') { alert('Enter a valid subreddit'); return; }
  
  document.getElementById('new-generate-btn').style.display = 'none';
  document.getElementById('new-generating-state').classList.remove('hidden');
  document.getElementById('new-subreddit-input').disabled = true;
  document.getElementById('new-persona-slider').disabled = true;
  
  try {
    const resp = await fetch('/api/communities/generate', {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        subreddit: subreddit,
        persona_count: parseInt(document.getElementById('new-persona-slider').value) || 18,
      }),
    });
    if (!resp.ok) {
      const body = await resp.json().catch(() => ({}));
      throw new Error(body.detail || 'Generation failed');
    }
    const data = await resp.json();
    activeCommunityId = data.community_id;
    currentPersonas = data.profiles;
    
    document.getElementById('new-generating-state').classList.add('hidden');
    document.getElementById('step1-num').classList.remove('active');
    document.getElementById('step1-num').classList.add('done');
    document.getElementById('step1-num').innerHTML = '&#10003;';
    
    document.getElementById('step2-num').classList.add('active');
    document.getElementById('step2-title').style.color = 'var(--text)';
    
    document.getElementById('new-personas-section').classList.remove('hidden');
    document.getElementById('new-persona-count').textContent = data.profile_count;
    
    renderNewPersonas(currentPersonas);
    
  } catch (err) {
    alert('Failed: ' + err.message);
    document.getElementById('new-generate-btn').style.display = 'block';
    document.getElementById('new-generating-state').classList.add('hidden');
    document.getElementById('new-subreddit-input').disabled = false;
    document.getElementById('new-persona-slider').disabled = false;
  }
});

function renderNewPersonas(profiles) {
  const container = document.getElementById('new-personas-container');
  container.innerHTML = profiles.map(p => {
    const flairColor = getFlairColor(p.archetype);
    const bioText = p.bio || '';
    return `
      <div class="persona-card" data-id="${p.id}">
        <div class="persona-header">
          <span class="persona-name">${escapeHTML(p.realname || p.username)}</span>
          <span class="persona-archetype" style="background-color:${flairColor}">${escapeHTML(p.archetype)}</span>
        </div>
        <div class="persona-bio">${escapeHTML(bioText)}</div>
        <div class="persona-actions">
          <button type="button" onclick="event.stopPropagation(); openEditModal(${p.id}, true)">Edit</button>
          <button type="button" onclick="event.stopPropagation(); deletePersona(${p.id}, true)">Remove</button>
        </div>
      </div>
    `;
  }).join('');
}

    
async function initSimulateView(communityId) {
  activeCommunityId = communityId;
  
  const [commResp, productResp] = await Promise.all([
    fetch('/api/communities'),
    fetch('/api/product'),
  ]);
  if (commResp.ok) {
    const communities = await commResp.json();
    const community = communities.find(c => c.id === communityId);
    if (community) {
      document.getElementById('sim-community-name').textContent = community.subreddit;
      document.getElementById('sim-persona-count').textContent = community.profile_count + ' personas';
    }
  }
  
  if (productResp.ok) {
    const product = await productResp.json();
    if (product.llm_model) llmModelInput.value = product.llm_model;
    if (product.llm_base_url) baseUrlInput.value = product.llm_base_url;
    if (product.llm_api_key) apiKeyInput.value = product.llm_api_key;
    if (product.batch_size != null) batchSizeInput.value = product.batch_size;
  }
  
  const params = new URLSearchParams(window.location.search);
  const runTag = params.get('run');
  if (runTag) {
    loadRun(runTag);
  } else {
    activeRunTag = null;
    postReadonly.classList.add('hidden');
    postContent.classList.remove('hidden');
    launchBtn.classList.remove('hidden');
    newSimBtn.classList.add('hidden');
  }
  
  loadRunHistory(communityId);
}


    // Elements
    const agentsSlider = document.getElementById('agents-slider');
    const hoursSlider = document.getElementById('hours-slider');
    const agentsVal = document.getElementById('agents-val');
    const hoursVal = document.getElementById('hours-val');
    const calcInfo = document.getElementById('calc-info');
    const postContent = document.getElementById('post-content');
    const postReadonly = document.getElementById('post-readonly');
    const postLabel = document.getElementById('post-label');
    const newSimBtn = document.getElementById('new-sim-btn');
    const apiKeyInput = document.getElementById('api-key');
    const baseUrlInput = document.getElementById('base-url');
    const llmModelInput = document.getElementById('llm-model');
    const batchSizeInput = document.getElementById('batch-size');
    const launchBtn = document.getElementById('launch-btn');

    const subredditInput = document.getElementById('new-subreddit-input');
    const generateBtn = document.getElementById('new-generate-btn');
    const communityStatus = { textContent: '', style: {} };
    const noRedditNotice = { classList: { remove: () => {} } };
    const personasSection = document.getElementById('new-personas-section');
    const personasContainer = document.getElementById('new-personas-container');
    const personaCountEl = document.getElementById('new-persona-count');
    const togglePersonasBtn = document.getElementById('dummy-toggle');
    const editModal = document.getElementById('edit-modal');
    const editProfileId = document.getElementById('edit-profile-id');
    const editUsername = document.getElementById('edit-username');
    const editRealname = document.getElementById('edit-realname');
    const editArchetype = document.getElementById('edit-archetype');
    const editBio = document.getElementById('edit-bio');
    const editPersona = document.getElementById('edit-persona');
    const editCancel = document.getElementById('edit-cancel');
    const editSave = document.getElementById('edit-save');

    let activeCommunityId = 1;
    let redditKeysConfigured = false;
    let currentPersonas = [];
    
    const progressPanel = document.getElementById('progress-panel');
    const progressTitle = document.getElementById('progress-title');
    const progressSpinner = document.getElementById('progress-spinner');

    const progressStrip = document.getElementById('progress-strip');
    const progressPill = document.getElementById('progress-pill');
    const progressFill = document.getElementById('progress-fill');
    const progressStep = document.getElementById('progress-step');
    const progressCalls = document.getElementById('progress-calls');
    const logContainer = document.getElementById('log-container');
    const runList = document.getElementById('run-list');
    
    const resultsPanel = document.getElementById('results-panel');
    const tabs = document.querySelectorAll('.tab');
    const tabContents = document.querySelectorAll('.tab-content');
    
    const noCommentsMsg = document.getElementById('no-comments-msg');
    const statsBar = document.getElementById('stats-bar');
    const threadContainer = document.getElementById('thread-container');
    const analyzeBtn = document.getElementById('analyze-btn');
    const scorecardEl = document.getElementById('scorecard');
    const rewriteBtn = document.getElementById('rewrite-btn');
    const rewriteResult = document.getElementById('rewrite-result');
    const improvedPost = document.getElementById('improved-post');
    const resimulateBtn = document.getElementById('resimulate-btn');

    let ws = null;

    // Sliders & Calculation
    function updateCalc() {
      const agents = parseInt(agentsSlider.value);
      const hours = parseInt(hoursSlider.value);
      
      agentsVal.textContent = agents;
      hoursVal.textContent = hours;
      
      const rounds = hours * 2;
      const llmCalls = Math.floor(agents * rounds * 0.5);
      const minutes = Math.max(1, Math.round((llmCalls * 3) / 60));
      
      calcInfo.textContent = `${rounds} rounds · ~${llmCalls} LLM calls · ~${minutes} minutes`;
      
      updateSliderTrack(agentsSlider);
      updateSliderTrack(hoursSlider);
    }

    function updateSliderTrack(slider) {
      const percentage = (slider.value - slider.min) / (slider.max - slider.min) * 100;
      slider.style.background = `linear-gradient(to right, var(--brand) ${percentage}%, var(--border) ${percentage}%)`;
    }

    agentsSlider.addEventListener('input', updateCalc);
    hoursSlider.addEventListener('input', updateCalc);
    updateCalc();

    async function checkRedditStatus() {
      try {
        const resp = await fetch('/api/reddit-status');
        const data = await resp.json();
        redditKeysConfigured = data.configured;
        if (!redditKeysConfigured) {
          const genBtn = document.getElementById('new-generate-btn');
          const subInput = document.getElementById('new-subreddit-input');
          if (genBtn) {
            genBtn.disabled = true;
            genBtn.style.opacity = '0.5';
            genBtn.textContent = 'No Reddit API keys configured';
          }
          if (subInput) subInput.disabled = true;
        }
      } catch (e) {
        console.error('Failed to check reddit status:', e);
      }
    }

    async function loadDefaultCommunity() {
      try {
        const resp = await fetch('/api/communities/1/profiles');
        const profiles = await resp.json();
        currentPersonas = profiles;
        activeCommunityId = 1;
        renderPersonas(profiles);
      } catch (e) {
        console.error('Failed to load default community:', e);
      }
    }

    
    function renderPersonas(profiles) {
      personasSection.classList.remove('hidden');
      personaCountEl.textContent = profiles.length;

      const archetypes = {};
      profiles.forEach(p => {
        archetypes[p.archetype] = (archetypes[p.archetype] || 0) + 1;
      });

      document.getElementById('new-personas-container').innerHTML = profiles.map(p => {
        const flairColor = getFlairColor(p.archetype);
        const bioText = p.bio || '';
        const personaPreview = (p.persona || '').substring(0, 100) + (p.persona && p.persona.length > 100 ? '...' : '');
        return '<div class="persona-card" data-id="' + p.id + '">' +
          '<div class="persona-card-header">' +
            '<span class="persona-card-name">' + escapeHTML(p.realname || p.username) + '</span>' +
            '<span class="persona-card-archetype" style="background-color:' + flairColor + '">' + escapeHTML(p.archetype) + '</span>' +
          '</div>' +
          '<div class="persona-card-bio">' + escapeHTML(bioText) + '</div>' +
          (personaPreview ? '<div class="persona-card-bio" style="margin-top:4px;font-style:italic;">' + escapeHTML(personaPreview) + '</div>' : '') +
          '<div class="persona-card-actions">' +
            '<button type="button" onclick="event.stopPropagation(); openEditModal(' + p.id + ')">Edit</button>' +
            '<button type="button" class="delete-btn" onclick="event.stopPropagation(); deletePersona(' + p.id + ')">Delete</button>' +
          '</div>' +
        '</div>';
      }).join('');
    }

    
    function openEditModal(profileId) {
      const profile = currentPersonas.find(p => p.id === profileId);
      if (!profile) return;
      editProfileId.value = profileId;
      editUsername.value = profile.username || '';
      editRealname.value = profile.realname || '';
      editArchetype.value = profile.archetype || '';
      editBio.value = profile.bio || '';
      editPersona.value = profile.persona || '';
      editModal.classList.remove('hidden');
    }

    editCancel.addEventListener('click', () => {
      editModal.classList.add('hidden');
    });

    editModal.addEventListener('click', (e) => {
      if (e.target === editModal) editModal.classList.add('hidden');
    });

    editSave.addEventListener('click', async () => {
      const profileId = parseInt(editProfileId.value);
      const fields = {
        username: editUsername.value.trim(),
        realname: editRealname.value.trim(),
        archetype: editArchetype.value.trim(),
        bio: editBio.value.trim(),
        persona: editPersona.value.trim(),
      };

      editSave.disabled = true;
      editSave.textContent = 'Saving...';
      try {
        const resp = await fetch('/api/communities/profiles/' + profileId, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify(fields),
        });
        if (!resp.ok) throw new Error('Update failed');

        const idx = currentPersonas.findIndex(p => p.id === profileId);
        if (idx !== -1) {
          Object.assign(currentPersonas[idx], fields);
        }
        
        if (document.getElementById('view-add-community').classList.contains('active')) {
          renderNewPersonas(currentPersonas);
        } else {
          renderPersonas(currentPersonas);
        }

        editModal.classList.add('hidden');
      } catch (err) {
        alert('Failed to save: ' + err.message);
      } finally {
        editSave.disabled = false;
        editSave.textContent = 'Save';
      }
    });

    async function deletePersona(profileId, isNewView = false) {
      if (!confirm('Delete this persona?')) return;
      try {
        await fetch('/api/communities/profiles/' + profileId, {method: 'DELETE'});
        currentPersonas = currentPersonas.filter(p => p.id !== profileId);
        if (isNewView) { renderNewPersonas(currentPersonas); } else { renderPersonas(currentPersonas); }
      } catch (e) {
        alert('Failed to delete persona');
      }
    }

    
    // Tabs logic
    tabs.forEach(tab => {
      tab.addEventListener('click', () => {
        tabs.forEach(t => t.classList.remove('active'));
        tabContents.forEach(c => c.classList.remove('active'));
        
        tab.classList.add('active');
        document.getElementById(tab.dataset.target).classList.add('active');
      });
    });

    // Logging
    function appendLog(msg) {
      const div = document.createElement('div');
      div.className = 'log-line';
      div.textContent = `> ${msg}`;
      logContainer.appendChild(div);
      logContainer.scrollTop = logContainer.scrollHeight;
    }


    function updateProgress(data) {
      progressStrip.classList.remove('hidden');

      const colors = {
        setup: 'var(--border)',
        simulation: 'var(--brand)',
        interview: 'var(--success)',
        complete: 'var(--success)',
      };
      const labels = {
        setup: 'SETUP',
        simulation: 'SIMULATING',
        interview: 'INTERVIEWING',
        complete: 'COMPLETE',
      };

      const color = colors[data.phase] || 'var(--border)';
      progressPill.textContent = labels[data.phase] || data.phase;
      progressPill.style.backgroundColor = color;
      progressFill.style.backgroundColor = color;

      let pct = 0;
      let step = '';

      if (data.phase === 'setup') {
        pct = 0;
        step = 'Preparing...';
      } else if (data.phase === 'simulation') {
        pct = (data.round / data.total_rounds) * 100;
        step = 'Round ' + data.round + '/' + data.total_rounds;
        if (data.hour) step += ' \u00b7 ' + data.hour;
        if (data.active_agents != null) step += ' \u00b7 ' + data.active_agents + ' agents';
      } else if (data.phase === 'interview') {
        pct = data.total > 0 ? (data.current / data.total) * 100 : 100;
        step = data.current + '/' + data.total;
        if (data.agent) step += ' \u00b7 ' + data.agent;
      } else if (data.phase === 'complete') {
        pct = 100;
        step = 'Done';
      }

      progressFill.style.width = pct + '%';
      progressStep.textContent = step;

      if (data.llm_calls != null) {
        progressCalls.textContent = '\u00b7 ' + data.llm_calls + ' calls';
      }
    }

    function connectWebSocket() {
      if (ws) {
        ws.close();
      }
      const protocol = location.protocol === 'https:' ? 'wss:' : 'ws:';
      ws = new WebSocket(`${protocol}//${location.host}/ws/progress`);
      
      ws.onmessage = (event) => {
        try {
          const data = JSON.parse(event.data);
          if (data.type === 'log') {
            appendLog(data.message);
          } else if (data.type === 'progress') {
            updateProgress(data);
          } else if (data.type === 'done') {
            simulationDone(data.tag);
          } else if (data.type === 'error') {
            appendLog(`ERROR: ${data.message}`);
            resetLaunchButton();
          }
        } catch(e) {
          appendLog(event.data);
        }
      };

      ws.onclose = () => {
        // Simple auto-reconnect if not done running
        if (launchBtn.disabled && !launchBtn.classList.contains('success')) {
          setTimeout(connectWebSocket, 1000);
        }
      };
    }

    // Launch
    newSimBtn.addEventListener('click', () => {
      navigateTo('/simulate/' + activeCommunityId);
    });

    launchBtn.addEventListener('click', async () => {
      const content = postContent.value.trim();
      if (!content) {
        alert("Please enter a post content");
        return;
      }

      // UI updates
      launchBtn.disabled = true;
      launchBtn.innerHTML = '<span class="spinner"></span> Running...';
      launchBtn.style.backgroundColor = 'var(--border)';
      launchBtn.style.color = 'var(--muted)';
      
      progressPanel.classList.remove('hidden');
      progressTitle.innerHTML = '<span class="spinner" id="progress-spinner"></span>Running Simulation...';
      progressTitle.style.color = '#fff';
      logContainer.innerHTML = '';

      progressStrip.classList.add('hidden');
      progressFill.style.width = '0%';
      progressCalls.textContent = '';
      statsBar.classList.add('hidden');
      threadContainer.innerHTML = '<div class="no-comments-yet">Simulation in progress...</div>';
      
      connectWebSocket();
      appendLog("Starting simulation...");

      try {
        const response = await fetch('/api/simulate', {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            agent_count: parseInt(agentsSlider.value),
            total_hours: parseInt(hoursSlider.value),
            post_content: content,
            community_id: activeCommunityId,
            llm_api_key: apiKeyInput.value.trim(),
            llm_base_url: baseUrlInput.value.trim(),
            llm_model: llmModelInput.value.trim(),
          })
        });
        
        if (!response.ok) {
          const body = await response.json().catch(() => ({}));
          throw new Error(body.detail || `Server error ${response.status}`);
        }
      } catch (err) {
        appendLog(`Error: ${err.message}`);
        resetLaunchButton();
      }
    });

    async function simulationDone(tag) {
      progressTitle.innerHTML = '✓ Simulation complete';
      progressTitle.style.color = 'var(--success)';
      
      launchBtn.classList.add('success');
      launchBtn.textContent = '✓ Complete';
      launchBtn.style.backgroundColor = 'var(--success)';
      launchBtn.style.color = '#fff';
      
      setTimeout(resetLaunchButton, 3000);
      
      activeRunTag = tag;
      history.replaceState(null, '', '/simulate/' + activeCommunityId + '?run=' + tag);
      await loadResults(tag);
      loadRunHistory(activeCommunityId);
    }

    function resetLaunchButton() {
      launchBtn.disabled = false;
      launchBtn.textContent = '🚀 Launch Simulation';
      launchBtn.classList.remove('success');
      launchBtn.style.backgroundColor = '';
      launchBtn.style.color = '';
    }

    // Results
    async function loadResults(tag) {
      try {
        const resp = await fetch('/api/results/' + (tag || activeRunTag));
        if (!resp.ok) throw new Error('Failed to load results');
        const results = await resp.json();
        renderThread(results);
      } catch (err) {
        appendLog(`Error loading results: ${err.message}`);
      }
    }

    function getFlairColor(archetype) {
      const lower = archetype.toLowerCase();
      if (lower.includes('early')) return 'var(--flair-early)';
      if (lower.includes('scaled')) return 'var(--flair-scaled)';
      if (lower.includes('pm')) return 'var(--flair-pm)';
      if (lower.includes('indie')) return 'var(--flair-indie)';
      if (lower.includes('hr') || lower.includes('people')) return 'var(--flair-hr)';
      if (lower.includes('regular')) return 'var(--flair-regular)';
      if (lower.includes('vc') || lower.includes('growth')) return 'var(--flair-vc)';
      if (lower.includes('lurker')) return 'var(--flair-lurker)';
      return '#757575';
    }

    function resolveArchetype(author, profiles) {
      if (!profiles || !author) return 'Unknown';
      const lower = (author || '').toLowerCase();
      for (const [key, val] of Object.entries(profiles)) {
        if (lower.includes(key) || lower === val.username) return val.archetype;
        if (val.username && lower.includes(val.username)) return val.archetype;
      }
      for (const [key, val] of Object.entries(profiles)) {
        const parts = val.username.split('_');
        if (parts.some(p => p.length > 2 && lower.includes(p))) return val.archetype;
      }
      return 'Unknown';
    }

    function renderThread(data) {
      if (!data || !data.comments) return;
      
      const stats = data.stats || {};
      document.getElementById('stat-score').textContent = stats.score || 0;
      document.getElementById('stat-comments').textContent = stats.total_comments || data.comments.length || 0;
      document.getElementById('stat-agents').textContent = stats.commenting_agents + '/' + stats.total_agents;
      statsBar.classList.remove('hidden');
      
      threadContainer.innerHTML = '';
      
      // Sort comments by created_at
      const comments = [...data.comments].sort((a, b) =>
        new Date(a.created_at || 0) - new Date(b.created_at || 0)
      );
      
      comments.forEach(c => {
        const div = document.createElement('div');
        div.className = 'comment';
        
        const voteScore = (c.likes || 0) - (c.dislikes || 0);
        const archetype = resolveArchetype(c.author, data.profiles);
        const flairColor = getFlairColor(archetype);
        
        const timeStr = c.created_at
          ? new Date(c.created_at).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})
          : 'just now';
        
        div.innerHTML = `
          <div class="vote-column">
            <span class="upvote">▲</span>
            <span>${voteScore}</span>
            <span class="downvote">▼</span>
          </div>
          <div class="comment-content">
            <div class="comment-meta">
              <span class="author">${escapeHTML(c.author || 'User')}</span>
              <span class="flair" style="background-color: ${flairColor}">${archetype}</span>
              <span class="time">${timeStr}</span>
            </div>
            <div class="text">${escapeHTML(c.content || '')}</div>
          </div>
        `;
        threadContainer.appendChild(div);
      });
    }

    analyzeBtn.addEventListener('click', async () => {
      analyzeBtn.disabled = true;
      analyzeBtn.innerHTML = '<span class="spinner"></span> Analyzing...';

      try {
        if (!activeRunTag) { alert('No run selected'); analyzeBtn.disabled = false; analyzeBtn.textContent = '📊 Generate Scorecard'; return; }
        const data = await fetch('/api/scorecard/' + activeRunTag, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({batch_size: parseInt(batchSizeInput.value) || 0}),
        }).then(r => r.json());
        analyzeBtn.style.display = 'none';
        scorecardEl.classList.remove('hidden');
        renderScorecard(data);
      } catch (err) {
        analyzeBtn.disabled = false;
        analyzeBtn.textContent = '📊 Generate Scorecard';
        alert('Analysis failed: ' + err.message);
      }
    });



    function renderScItem(item) {
      const quotes = item.quotes || [];
      if (!quotes.length) {
        return '<li>• ' + escapeHTML(item.name) + ' <span class="freq">(' + item.count + '×)</span></li>';
      }
      let quotesHtml = '';
      quotes.forEach(q => {
        const colonIdx = q.indexOf(': ');
        if (colonIdx > -1) {
          const author = q.substring(0, colonIdx);
          const text = q.substring(colonIdx + 2);
          quotesHtml += '<div class="sc-quote"><span class="sc-quote-author">' + escapeHTML(author) + '</span>: ' + escapeHTML(text) + '</div>';
        } else {
          quotesHtml += '<div class="sc-quote">' + escapeHTML(q) + '</div>';
        }
      });
      return '<li><details class="sc-item"><summary>' + escapeHTML(item.name) + ' <span class="freq">(' + item.count + '×)</span></summary>' + quotesHtml + '</details></li>';
    }

    function renderScorecard(data) {
      const gradeEl = document.getElementById('sc-grade');
      gradeEl.textContent = data.grade;
      gradeEl.className = 'grade-circle';
      if (['A+', 'A', 'B+', 'B'].includes(data.grade)) gradeEl.classList.add('green');
      else if (['C+', 'C'].includes(data.grade)) gradeEl.classList.add('yellow');
      else gradeEl.classList.add('red');

      document.getElementById('sc-score').textContent = data.score + '/10';
      document.getElementById('sc-summary').textContent = data.summary;

      const bios = data.archetype_bios || {};
      const tbody = document.querySelector('#sc-matrix tbody');
      tbody.innerHTML = '';
      for (const [arch, counts] of Object.entries(data.matrix)) {
        const row = document.createElement('tr');
        const silent = counts.silent || 0;
        const bio = bios[arch] ? escapeHTML(bios[arch]) : 'Simulated Reddit persona';
        row.innerHTML =
          '<td><span data-tip="' + bio + '">' + arch + '</span></td>' +
          '<td>' + (counts.positive || 0) + '</td>' +
          '<td>' + (counts.neutral || 0) + '</td>' +
          '<td>' + (counts.negative || 0) + '</td>' +
          '<td class="silent">' + (silent > 0 ? silent : '') + '</td>';
        tbody.appendChild(row);
      }

      const funnelSection = document.getElementById('sc-conversion-section');
      const funnelEl = document.getElementById('sc-conversion');
      if (data.click_through && data.signup_funnel) {
        funnelSection.classList.remove('hidden');
        let clickPills = '';
        if (data.click_through.by_archetype) {
          for (const [arch, pct] of Object.entries(data.click_through.by_archetype)) {
            if (pct > 0) clickPills += '<span class="funnel-pill">' + escapeHTML(arch) + ' ' + pct + '%</span>';
          }
        }
        let signupPills = '';
        if (data.signup_funnel.by_archetype) {
          for (const [arch, pct] of Object.entries(data.signup_funnel.by_archetype)) {
            if (pct > 0) signupPills += '<span class="funnel-pill">' + escapeHTML(arch) + ' ' + pct + '%</span>';
          }
        }
        const ct = data.click_through;
        const sf = data.signup_funnel;
        const clickLabel = (ct.count || 0) + ' of ' + (ct.total || '?') + ' agents (' + ct.rate + '%)';
        const signupLabel = (sf.would_signup_count || 0) + ' of ' + (sf.total || '?') + ' agents (' + sf.would_signup_pct + '%)';
        funnelEl.innerHTML = 
          '<div class="funnel-row">' +
            '<div class="funnel-label"><span>Would Click Link</span><span>' + clickLabel + '</span></div>' +
            '<div class="funnel-bar-bg"><div class="funnel-bar-fill" style="width: ' + ct.rate + '%; background: var(--success);"></div></div>' +
            '<div class="funnel-pills">' + clickPills + '</div>' +
          '</div>' +
          '<div class="funnel-row">' +
            '<div class="funnel-label"><span>Would Sign Up</span><span>' + signupLabel + '</span></div>' +
            '<div class="funnel-bar-bg"><div class="funnel-bar-fill" style="width: ' + sf.would_signup_pct + '%; background: #2196F3;"></div></div>' +
            '<div class="funnel-pills">' + signupPills + '</div>' +
          '</div>';
      }

      const claritySection = document.getElementById('sc-clarity-section');
      const clarityEl = document.getElementById('sc-clarity');
      if (data.message_clarity) {
        claritySection.classList.remove('hidden');
        const mc = data.message_clarity;
        let html =
          '<div class="clarity-top">' +
            '<div class="clarity-score">' + mc.score + '%</div>' +
            '<div class="clarity-label">accurately described the product</div>' +
          '</div>' +
          '<div class="clarity-boxes">' +
            '<div class="clarity-box clear">Accurate: ' + (mc.accurate || mc.clear || 0) + '</div>' +
            '<div class="clarity-box partial">Partial: ' + mc.partial + '</div>' +
            '<div class="clarity-box confused">Wrong: ' + (mc.wrong || mc.confused || 0) + '</div>' +
          '</div>';

        if (mc.responses && mc.responses.length) {
          html += '<div style="margin-top: 16px;">';
          mc.responses.forEach(r => {
            let color = 'var(--success)';
            if (r.clarity === 'partial') color = '#f9a825';
            if (r.clarity === 'wrong') color = 'var(--error)';
            html +=
              '<details class="sc-item">' +
                '<summary>' +
                  '<span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:' + color + ';margin-right:8px;"></span>' +
                  escapeHTML(r.username) +
                  ' <span class="freq" style="color:' + color + '">(' + r.clarity + ')</span>' +
                '</summary>' +
                '<div class="sc-quote">' + escapeHTML(r.response) + '</div>' +
              '</details>';
          });
          html += '</div>';
        }

        clarityEl.innerHTML = html;
      }

      const objSection = document.getElementById('sc-objections-section');
      const objEl = document.getElementById('sc-objections');
      if (data.objection_map) {
        objSection.classList.remove('hidden');
        objEl.innerHTML = '';
        if (data.objection_map.length) {
          data.objection_map.forEach(obj => {
            let quotesHtml = '';
            const items = obj.items || [];
            items.slice(0, 3).forEach(q => {
              quotesHtml += '<div class="sc-quote"><span class="sc-quote-author">' + escapeHTML(q.author || '') + '</span>: ' + escapeHTML(q.text || '') + '</div>';
            });
            
            let badgeClass = 'badge-technical';
            if (obj.type === 'trust') badgeClass = 'badge-trust';
            if (obj.type === 'value') badgeClass = 'badge-value';
            if (obj.type === 'pricing') badgeClass = 'badge-pricing';

            objEl.innerHTML += 
              '<div class="sc-item">' +
                '<details>' +
                  '<summary>' +
                    '<span class="objection-badge ' + badgeClass + '">' + escapeHTML(obj.type) + '</span>' +
                    obj.count + ' objection' + (obj.count !== 1 ? 's' : '') +
                  '</summary>' +
                  quotesHtml +
                '</details>' +
              '</div>';
          });
        } else {
          objEl.innerHTML = '<div style="color:var(--muted);font-size:13px;">None detected</div>';
        }
      }

      const audSection = document.getElementById('sc-audience-section');
      const audBody = document.querySelector('#sc-audience tbody');
      if (data.audience_fit) {
        audSection.classList.remove('hidden');
        audBody.innerHTML = '';
        if (data.audience_fit.length) {
          data.audience_fit.forEach(row => {
            const tr = document.createElement('tr');
            let color = 'var(--error)';
            if (row.positive_rate > 60) color = 'var(--success)';
            else if (row.positive_rate > 30) color = '#f9a825';
            
            tr.innerHTML = 
              '<td>' + escapeHTML(row.archetype) + '</td>' +
              '<td>' + (row.agent_count || '') + '</td>' +
              '<td style="color: ' + color + '; font-weight: bold;">' + (row.comment_count > 0 ? row.positive_rate + '%' : '-') + '</td>' +
              '<td>' + row.comment_count + '</td>' +
              '<td>' + (row.avg_length || '-') + '</td>';
            audBody.appendChild(tr);
          });
        }
      }

      const featSection = document.getElementById('sc-features-section');
      const featEl = document.getElementById('sc-features');
      featEl.innerHTML = '';
      if (data.missing_features && data.missing_features.length) {
        featSection.classList.remove('hidden');
        data.missing_features.forEach(f => {
          featEl.innerHTML += renderScItem(f);
        });
      }
    }

    rewriteBtn.addEventListener('click', async () => {
      rewriteBtn.disabled = true;
      rewriteBtn.innerHTML = '<span class="spinner"></span> Rewriting...';

      try {
        if (!activeRunTag) { alert('No run selected'); rewriteBtn.disabled = false; rewriteBtn.textContent = '✍️ Rewrite Post'; return; }
        const data = await fetch('/api/rewrite/' + activeRunTag, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({batch_size: parseInt(batchSizeInput.value) || 0}),
        }).then(r => r.json());
        rewriteResult.classList.remove('hidden');
        improvedPost.value = data.improved_post || '';
        rewriteBtn.style.display = 'none';
      } catch (err) {
        rewriteBtn.disabled = false;
        rewriteBtn.textContent = '🔄 Rewrite Post Based on Feedback';
        alert('Rewrite failed: ' + err.message);
      }
    });

    resimulateBtn.addEventListener('click', () => {
      postContent.value = improvedPost.value;
      window.scrollTo({top: 0, behavior: 'smooth'});
      tabs[0].click();
      launchBtn.click();
    });

    function escapeHTML(str) {
      return str.replace(/[&<>'"]/g,
        tag => ({
            '&': '&amp;',
            '<': '&lt;',
            '>': '&gt;',
            "'": '&#39;',
            '"': '&quot;'
          }[tag]));
    }

    let activeRunTag = null;

    async function loadRunHistory(communityId) {
      try {
        
    let url = '/api/runs';
    if (communityId) url += '?community_id=' + communityId;
    const resp = await fetch(url);

        const runs = await resp.json();
        renderRunList(runs);
      } catch(e) {
        console.error('Failed to load run history:', e);
      }
    }

    function renderRunList(runs) {
      if (!runs.length) {
        runList.innerHTML = '<div style="font-size:12px;color:var(--muted);padding:8px;">No runs yet</div>';
        return;
      }
      runList.innerHTML = runs.map(run => {
        const isActive = run.tag === activeRunTag ? ' active' : '';
        const date = new Date(run.created_at).toLocaleDateString();
        const time = new Date(run.created_at).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
        const comments = run.comment_count || 0;
        return '<div class="run-item' + isActive + '" data-tag="' + run.tag + '" onclick="loadRun(\'' + run.tag + '\')">' +
          '<div class="run-item-info">' +
          '  <div class="run-item-tag"><span class="run-item-status ' + run.status + '"></span>' + run.tag + '</div>' +
          '  <div class="run-item-meta">' + run.agent_count + ' agents · ' + comments + ' comments · ' + date + ' ' + time + '</div>' +
          '</div>' +
          '<button class="run-item-delete" onclick="event.stopPropagation(); deleteRun(\'' + run.tag + '\')" title="Delete run">&times;</button>' +
          '</div>';
      }).join('');
    }

    async function loadRun(tag) {
      activeRunTag = tag;
      try {
        const resp = await fetch('/api/results/' + tag);
        if (!resp.ok) throw new Error('Failed to load results');
        const data = await resp.json();
        renderThread(data);
        postContent.classList.add('hidden');
        postReadonly.textContent = data.post.content || '';
        postReadonly.classList.remove('hidden');
        postLabel.textContent = 'Post (' + tag + ')';
        launchBtn.classList.add('hidden');
        newSimBtn.classList.remove('hidden');
                if (typeof rewriteResult !== 'undefined') rewriteResult.classList.add('hidden');
        if (typeof rewriteBtn !== 'undefined') { rewriteBtn.style.display = ''; rewriteBtn.disabled = false; }
        loadRunHistory(activeCommunityId);

        const scResp = await fetch('/api/scorecard/' + tag, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({batch_size: parseInt(batchSizeInput.value) || 0}),
        });
        if (scResp.ok) {
          const scData = await scResp.json();
          analyzeBtn.style.display = 'none';
          scorecardEl.classList.remove('hidden');
          renderScorecard(scData);
        } else {
          scorecardEl.classList.add('hidden');
          analyzeBtn.style.display = '';
          analyzeBtn.disabled = false;
          analyzeBtn.textContent = '\uD83D\uDCCA Generate Scorecard';
        }
      } catch(e) {
        console.error('Failed to load run:', e);
      }
    }

    async function deleteRun(tag) {
      if (!confirm('Delete this run?')) return;
      try {
        await fetch('/api/runs/' + tag, {method: 'DELETE'});
        if (activeRunTag === tag) {
          activeRunTag = null;
          threadContainer.innerHTML = '<div class="no-comments-yet">Launch a simulation to see agent reactions</div>';
          statsBar.classList.add('hidden');
        }
        loadRunHistory(activeCommunityId);
      } catch(e) {
        console.error('Failed to delete run:', e);
      }
    }

    routeToView();
